...
describe('file system checks', function () {
  ...
  describe.skip('Consistent data in aux fils', function () {
    it('should contain identical description strings', function (done) {
    ...
    it('should contain identical version number', function (done) {
      if (fs.existsSync('package.json')) {
        const pkg = fs.readFileSync('package.json', 'utf8')
        const parsed = JSON.parse(pkg)
    ...
    it('should contain identical title', function (done) {
      if (fs.existsSync('build.xml')) {
        const build = fs.readFileSync('build.xml', 'utf8')
        const parsed = new xmldoc.XmlDocument(build)
    ...
    // TODO add licenese consistency check
